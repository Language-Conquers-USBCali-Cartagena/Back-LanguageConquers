...
public interface RetoEstudianteDAO extends JpaRepository<RetoEstudiante,Long> {
    ...
    @Query(value = "SELECT AVG(retos_completados) as promedio_retos_completados FROM (select count(*) as retos_completados from reto_estudiante where id_estado = 2 group by id_estudiante) as t", nativeQuery = true)
